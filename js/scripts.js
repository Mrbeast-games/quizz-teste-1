...
let objetivo = 421.50; // valor do poup que você deseja atingir
let velocidade = 0.01; // velocidade do contador
let contadorAtual = 0; // inicializar o contador em zero

function startCounter() {
  const intervalId = setInterval(() => {    if (contadorAtual < objetivo) {
      contadorAtual += velocidade;
      counter.innerText = contadorAtual.toFixed(2);
    } else {
      clearInterval(intervalId);
    }
  }, 1); // ajuste o tempo de intervalo aqui (em milissegundos)
}

// Iniciar o contador automaticamente quando o popup for carregado
startCounter();
